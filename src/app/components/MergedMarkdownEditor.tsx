...
const MergedMarkdownEditor: React.FC<MarkdownEditorProps> = ({ documentId, workspaceId, doc, provider, onDocumentSaved }) => {
  const { user } = useUser();
  ...
  const compileMarkdown = useCallback(async () => {
    ...
    try {
      ...
      setCompiledContent(processMarkdown(markdownContent));
  ...
  }, [markdownContent, isCompiling, processMarkdown]);
...
return sanitizeHtml(renderedHtml, {
  allowedTags: [
    "h1", "h2", "h3", "p", "a", "ul", "ol", "li", "blockquote",
    "code", "pre", "hr", "strong", "em", "div", "span", "del",
    "sup", "sub", "section", "footer", "abbr", "table", "thead", "tbody", "tr", "td", "th",
    "script", "math", "semantics", "mrow", "mi", "mo", "mn", "msup", "msub", "mfrac", "msqrt", "mroot"
  ],
      allowedAttributes: {
    ...
    <MathJaxContext config={mathJaxConfig}>
      ...
      >
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          {showPreview && (
            <div
              className={`w-1/2 h-full flex flex-col border-l transition-all duration-300 ${
                darkMode
                  ? "bg-gray-800 border-gray-700"
